...
def test_text_change():
    ...
    diff = SimpleEditTypes(prev_wikitext, curr_wikitext, lang='en', tokenizer=enTokenizer).get_diff()
    assert diff == expected_changes
    full_diff = StructuredEditTypes(prev_wikitext, curr_wikitext, lang='en', tokenizer=enTokenizer).get_diff()
...
def test_empty_previous():
    ...
    diff = SimpleEditTypes('', curr_wikitext, lang='en', tokenizer=enTokenizer).get_diff()
    assert diff == expected_changes
    full_diff = StructuredEditTypes('', curr_wikitext, lang='en', tokenizer=enTokenizer).get_diff()
    assert full_diff_to_simple(full_diff) == expected_changes
