...
class StructuredEditTypes:
    def __init__(
        self, prev_wikitext="", curr_wikitext="", lang="en", timeout=False, debug=False, tokenizer=None
    ):
    ...
    def get_diff(self):
        ...
        self.actions = get_diff_count(self.tree_diff, lang=self.lang, tokenizer=self.tokenizer)
        return self.actions


class SimpleEditTypes:
    def __init__(self, prev_wikitext="", curr_wikitext="", lang="en", tokenizer=None):
        self.prev_wikitext = prev_wikitext
        self.curr_wikitext = curr_wikitext
        self.lang = lang
        self.actions = None
    ...
    def get_diff(self):
        self.actions = simple_get_diff(
            self.prev_wikitext, self.curr_wikitext, lang=self.lang, tokenizer=self.tokenizer
        )
        return self.actions
...
class EditCategories:
    def __init__(self):
        ...
        self.MAINTENANCE_TYPES = [
            "List",  # this is just the syntax -- e.g., adding a `*` to the start of a line
            "Text Formatting",
            "Punctuation",  # text changes that don't really impact meaning
    ...
    def simple_et_to_higher_level(self, summary):
        ...
        for et in summary:
            ...
            elif et == "Punctuation" and (
                "Word" not in summary and "Character" not in summary
            ):
                types[self.CON_MAI] = types.get(self.CON_MAI, 0) + 1
